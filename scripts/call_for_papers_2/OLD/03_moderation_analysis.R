...
# --- Parametri empirici ricavati dal summary (scala originale) ----------------
vowel_means <- list(
  a = list(
    f0_mean = 189.9,
    f0_std = 6.230,
...
make_priors_vowel <- function(outcome, vowel) {
  ...
  # -------------------------------
  # Intercept priors (outcome-scale) - SCALA ORIGINALE, NON LOG
  # -------------------------------
  intercept_prior <- switch(
    ...
    # MODIFICATO: usa vm$f0_std direttamente, non log(vm$f0_std)
    "f0_std" = prior_string(
      paste0("student_t(3, ", fmt(vm$f0_std), ", 5)"),
      class = "Intercept"
    ),

    # MODIFICATO: usa vm$jitter direttamente, non log(vm$jitter)
    "jitter" = prior_string(
      paste0("student_t(3, ", fmt(vm$jitter), ", 0.5)"),
      class = "Intercept"
    ),
    ...
    # MODIFICATO: usa vm$f2_std direttamente, non log(vm$f2_std)
    "f2_std" = prior_string(
      paste0("student_t(3, ", fmt(vm$f2_std), ", 30)"),
      class = "Intercept"
    ),
  ...
  # -----------------------------------
  # Fixed effects: main vs interactions
  # -----------------------------------
  priors_fixed <- c(
    # Main effects (c1, c2, PID-5)
    prior_string("normal(0, 5)", class = "b"),

    # Stress reactivity interactions
    prior_string("normal(0, 2)", class = "b", coef = "c1_stress:"),

    # Recovery interactions
    prior_string("normal(0, 2)", class = "b", coef = "c2_recovery:")
  )
  ...
  # -------------------------
  priors_random <- c(
    prior_string("exponential(0.5)", class = "sd"),
    prior_string("exponential(0.5)", class = "sigma")
  )
...
# MODIFICATO: f2_mean ora usa gaussian() invece di asym_laplace()
outcome_families <- list(
  f0_mean = gaussian(),
  f0_std = gaussian(),
  jitter = gaussian(),
  nne = gaussian(),
  f2_mean = gaussian(),   # ERA: asym_laplace()
  f2_std = gaussian()
)
...
p_forest <- ggplot(
  ...
  labs(
    x = "Posterior Estimate (scala originale: Hz, %, etc.)",
    y = "Parametro",
    color = "Outcome",
    shape = "Significativo (CI esclude 0)",
    title = "Bayesian Forest Plot: Interazioni PID-5 × Stress",
    subtitle = "VERSIONE ORIGINALE (senza trasformazione log)"
  ) +
  theme(
...
cat("\n")
cat(rep("=", 70), "\n", sep = "")
cat("ANALISI COMPLETATA (VERSIONE ORIGINALE)\n")
cat("I coefficienti sono interpretabili direttamente in Hz, %, etc.\n")
cat(rep("=", 70), "\n", sep = "")

# ==============================================================================
# NOTA INTERPRETATIVA
# ==============================================================================
# Con questa versione ORIGINALE (senza log-transform):
#
# - I coefficienti di f0_mean sono in Hz (es. β = 5.03 Hz significa che
#   per ogni SD di aumento in Negative Affectivity, il pitch aumenta
#   di 5.03 Hz durante lo stress)
#
# - I coefficienti di f2_mean/f2_std sono in Hz
#
# - I coefficienti di jitter sono in percentuale (%)
#
# - I coefficienti di nne sono in dB
#
# Questo corrisponde ai risultati riportati nell'abstract:
#   - NA amplifica aumenti di pitch: β = 5.03 Hz [1.26, 8.76]
#   - Detachment compromette recovery: β = -4.37 [-8.30, -0.46]
#   - Antagonism facilita recovery: β = 3.44-4.17
