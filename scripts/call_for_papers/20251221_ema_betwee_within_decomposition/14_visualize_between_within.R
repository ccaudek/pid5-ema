...
# Visualization of between-person vs within-person effects
# ==============================================================================
# PURPOSE:
#   Create publication-quality figures showing:
#   1. Decomposition of trait vs state variance
#   2. Forest plots comparing between vs within effects
#   3. Interaction plots for credible effects
...
p_variance <- ggplot(icc_long, aes(x = domain, y = percentage, fill = component)) +
  geom_col(position = "stack", alpha = 0.8) +
  geom_text(aes(label = sprintf("%.0f%%", percentage)),
            position = position_stack(vjust = 0.5),
            size = 3.5, fontface = "bold") +
  scale_fill_manual(
    values = c("Between-person (Trait)" = "#2E86AB", 
...
ggsave("figures/between_within/variance_decomposition.png", 
...
forest_data <- comparison %>%
  select(domain, 
         starts_with("between_main"), starts_with("within_main"),
         starts_with("between_stress"), starts_with("within_stress"),
         starts_with("between_recovery"), starts_with("within_recovery")) %>%
  pivot_longer(
    cols = -domain,
...
p_forest <- ggplot(forest_main, 
  ...
  scale_shape_manual(
    ...
    name = "Credible",
    labels = c("FALSE" = "No (95% CI includes 0)", 
...
ggsave("figures/between_within/forest_plot_main_effects.png",
...
if (nrow(credible_within) > 0) {
  ...
  plot_interaction <- function(domain_name, effect_type) {
    ...
    pred_grid <- pred_grid %>%
      mutate(
        ...
        state_label = factor(state_level,
                            levels = c(-1, 0, 1),
    ...
    # Plot
    p <- ggplot(preds, aes(x = timepoint, y = Estimate, 
                           color = state_label, group = state_label)) +
      geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5, fill = state_label),
                  alpha = 0.2, color = NA) +
  ...
  for (i in 1:nrow(credible_within)) {
    domain_name <- str_to_title(str_replace_all(credible_within$domain[i], "_", " "))
    
    if (credible_within$within_stress_credible[i] | 
        credible_within$within_recovery_credible[i]) {
      
      p <- plot_interaction(domain_name, "combined")
      interaction_plots[[domain_name]] <- p
      
      filename <- paste0("figures/between_within/interaction_",
                        tolower(str_replace_all(domain_name, " ", "_")), ".png")
...
p_r2 <- ggplot(r2_data, aes(x = model, y = value)) +
  ...
  geom_text(aes(label = sprintf("R² = %.3f", value)),
...
ggsave("figures/between_within/model_comparison_r2.png",
       p_r2, width = 8, height = 6, dpi = 300)
...
ggsave("figures/between_within/combined_summary.png",
...
pub_table <- comparison %>%
  mutate(
    ...
    # Between main
    between_main = sprintf("%.2f [%.2f, %.2f]%s",
                          between_main_mean, between_main_ci_lower, 
                          between_main_ci_upper,
                          ifelse(between_main_credible, "*", "")),
    
    # Within main
    within_main = sprintf("%.2f [%.2f, %.2f]%s",
                         within_main_mean, within_main_ci_lower,
                         within_main_ci_upper,
                         ifelse(within_main_credible, "*", "")),
    
    # Between stress
    between_stress = sprintf("%.2f [%.2f, %.2f]%s",
                            between_stress_mean, between_stress_ci_lower,
                            between_stress_ci_upper,
                            ifelse(between_stress_credible, "*", "")),
    
    # Within stress
    within_stress = sprintf("%.2f [%.2f, %.2f]%s",
                           within_stress_mean, within_stress_ci_lower,
...
if (length(interaction_plots) > 0) {
  cat("  5. interaction_*.png - Credible state × timepoint effects\n")
}
