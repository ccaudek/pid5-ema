...
model {
  ...
  sigma_pos ~ exponential(5);
  sigma_neg ~ exponential(5);
...
generated quantities {
  ...
  // ---- PSIS-LOO marginalizzato per osservazione (blocchi di 4 item per beep) ----
  // Media Monte Carlo sul prior condizionale degli effetti specifici dell'osservazione:
  //   u_pos[n] ~ Normal(0, sigma_pos), u_neg[n] ~ Normal(0, sigma_neg), eps ~ Student-t(nu_state, 0, 1)
  // Nota: assumiamo che i 4 item dell'osservazione n siano ordinati contiguamente in y_item
  //       come costruito in R: indici ((n-1)*K + 1) ... (n*K).
  int S_prior_obs = 16;   // puoi 8/16/32; 16 di solito basta
  vector[N_obs] log_lik_obs_marg;
  for (n in 1:N_obs) {
    int i = subject[n];
    int p = period[n];
    int start = (n - 1) * K + 1;
    ...
    // parti sempre da -Inf per log-sum-exp
    real lse = negative_infinity();

    // componenti deterministiche (senza eps) dell'eta
    real m_pre  = (p == 2) ? (gamma_pre  + delta_pre[i])  : 0.0;
    real m_post = (p == 3) ? (gamma_post + delta_post[i]) : 0.0;
    real eta_fixed = theta[i, 1] + m_pre + m_post;

    for (s in 1:S_prior_obs) {
      real u_p  = normal_rng(0, sigma_pos);
      real u_n  = normal_rng(0, sigma_neg);
      real epsn = student_t_rng(nu_state, 0, 1);
      real eta_n = eta_fixed + sigma_state * epsn;

      // somma delle log-lik dei 4 item dell'osservazione n
      real ll_sum = 0;
      for (idx in start:stop) {
        int k = item_id[idx];
        real m = (k == 1 || k == 3) ? u_p : u_n; // pos per happy/satisfied; neg per sad/angry
        real lp = nu[k] + lambda[k] * eta_n + m;
        ll_sum += ordered_logistic_lpmf(y_item[idx] | lp, tau[k]);
      }
      lse = log_sum_exp(lse, ll_sum);
    }
    log_lik_obs_marg[n] = lse - log(S_prior_obs);
  }

    // --- Posterior predictive: item-level repliche e fragilità replicata ---
  array[N_items] int y_item_rep;
  vector[I] diff_frag_rep;

  {
    // repliche degli item
    for (n in 1:N_items) {
      int k = item_id[n];
      real m = (k == 1 || k == 3) ? u_pos[obs_id[n]] : u_neg[obs_id[n]];
      real lp = nu[k] + lambda[k] * eta[obs_id[n]] + m;
      // ordered_logistic_rng restituisce una categoria 1..7
      y_item_rep[n] = ordered_logistic_rng(lp, tau[item_id[n]]);
    }
  }

  // repliche della fragilità (pre - post) a livello soggetto
  for (i in 1:I) {
    diff_frag_rep[i] = student_t_rng(nu_diff, mu_frag_hat[i], sigma_diff);
  }
