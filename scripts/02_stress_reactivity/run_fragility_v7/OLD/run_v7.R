...
# A
ll_A_s <- fit_A$draws("log_lik_subj_gh", format = "matrix") # draws_tot x S
...
# PSIS-LOO (con moment matching) + confronto e diagnostiche
...
# b_mat:      nd x (S*R), colonne in ordine tipo Stan: b_subj[1,1], b_subj[2,1], ..., b_subj[S,1], b_subj[1,2], ...
# Z:          N x R  (design dei random effects)
# subj:       vettore lungo N con indici 1..S
# S, R:       numeri di soggetti e di random effects
# b_colnames: opzionale; se passato, riordina le colonne di b_mat in base a [r poi s] estraendo dagli header "b_subj[s,r]"
re_contrib_all <- function(b_mat, Z, subj, S, R, b_colnames = NULL) {
  stopifnot(is.matrix(b_mat), is.matrix(Z), length(subj) == nrow(Z))
  # (opzionale) riordina le colonne per sicurezza
  if (!is.null(b_colnames)) {
    ...
    if (all(ok)) {
      ...
      ord <- order(r_idx, s_idx) # ordina per r poi s
  ...
  for (d in seq_len(nd)) {
    # Ricostruisci B_s (S x R) in COLONNE (byrow = FALSE) → coherence con naming Stan
    Bsr <- matrix(as.numeric(b_mat[d, ]), nrow = S, ncol = R, byrow = FALSE)
    B_obs <- Bsr[subj, , drop = FALSE] # N x R (sceglie la riga del soggetto per ogni osservazione)
    out[d, ] <- rowSums(Z * B_obs) # prodotto riga per riga: Z[n,]*B_obs[n,]
  }
  out
}

# Oggetti dal tuo ambiente:
# data_B$X, data_B$Z, data_B$subj, data_B$y, fit_B
...
## (i) Densità globale (ok che non sia perfetta per via della scala discreta, vedi discussione)
bayesplot::ppc_dens_overlay(y, yrep[1:50, ])

## (ii) ECDF globale (calibrazione)
bayesplot::ppc_ecdf_overlay(y, yrep[1:50, ])

## (iii) QQ (sovrapposizione di più repliche)
...
## (iv) Target diagnostico: SD entro-soggetto
...
# PPC per media per soggetto (oltre alla SD):
ybar_obs <- tapply(data_B$y, data_B$subj, mean)
ybar_reps <- apply(yrep, 1, function(yr) tapply(yr, data_B$subj, mean))
bayesplot::ppc_dens_overlay(ybar_obs, t(ybar_reps)[1:50, ])

# PPC per periodo (baseline/pre/post):

# --- fattore soggetto×periodo, allineato a data_B ---
per <- df_mod$is_pre_bin + 2L * df_mod$is_post_bin
per <- per[data_B$rows_kept] # riallinea a data_B
grp <- interaction(data_B$subj, per, drop = TRUE)

# --- tieni solo i gruppi con almeno 2 osservazioni ---
n_by_grp <- tapply(data_B$y, grp, function(v) sum(is.finite(v)))
keep_lvls <- names(n_by_grp)[n_by_grp >= 2L]
f_keep <- factor(grp, levels = keep_lvls) # stesso set di livelli per obs e repliche
idx_keep <- !is.na(f_keep) # TRUE solo per gruppi tenuti

# --- sd osservate per gruppo (nessun NA ora) ---
sd_by_sp_obs <- tapply(data_B$y[idx_keep], f_keep[idx_keep], sd)

# --- sd replicate: matrice nd x G (nd = nrow(yrep); G = #gruppi tenuti) ---
G <- length(keep_lvls)
...
# --- PPC densità sulle sd per soggetto×periodo (50 repliche) ---
