...
stan_file_path <- here::here(
  ...
  "run_fragility_v3",
  "subject_loo_student_no_reduce_sum.stan"
)
SEED <- 20250919
...
# --- Gauss-Hermite (Q=20) ---
Q <- 20L
# usa fastGHQuad se presente, altrimenti statmod
...
} else if (requireNamespace("statmod", quietly = TRUE)) {
  gh <- statmod::gauss.quad(Q, kind = "hermite")
  gh_nodes <- as.numeric(gh$nodes)
  gh_weights <- as.numeric(gh$weights)
} else {
  stop("Installare 'fastGHQuad' o 'statmod' per i nodi Gauss-Hermite.")
}
...
# ------------------------------------------------------------
stan_file_t <- here::here(
  "scripts",
  "02_stress_reactivity",
  "run_fragility_v3",
  "subject_loo_student_no_reduce_sum.stan"
)
mod_t <- cmdstan_model(stan_file_t, force_recompile = TRUE)
