...
# ==== LOO per soggetto (MARGINALIZZATO) ====
...
# ==== PPC: ora i RE includono slopes, quindi la varianza entro-soggetto dovrebbe allinearsi meglio ====
library(posterior)
library(bayesplot)
...
# (facoltativo) PPC per periodo
is_pre_vec <- as.integer(df_mod$is_pre[data_B$rows_kept])
is_post_vec <- as.integer(df_mod$is_post[data_B$rows_kept])
bayesplot::ppc_dens_overlay(y[is_pre_vec == 1], yrep[, is_pre_vec == 1][1:50, ])
bayesplot::ppc_dens_overlay(
  y[is_post_vec == 1],
  yrep[, is_post_vec == 1][1:50, ]
)
